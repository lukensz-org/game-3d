...
Image createDepthImage(ResourceAllocator &allocator, VkFormat depth_format,
                       PixelExtent extent) {
...
// working public depth path, nearer geometry must still occlude farther
// geometry.
constexpr std::array kCubes{
    CubeInstance{{-0.35F, 0.05F, -1.6F},
                 {1.1F, 1.1F, 1.1F},
                 0.92F,
                 0.28F,
                 0.28F}, // nearer, red
    CubeInstance{{0.0F, 0.55F, -2.4F},
                 {0.9F, 0.9F, 0.9F},
                 0.28F,
                 0.78F,
                 0.42F}, // mid, green
    CubeInstance{{-0.8F, -0.55F, -2.8F},
                 {0.8F, 0.8F, 0.8F},
                 0.95F,
                 0.78F,
                 0.22F}, // yellow
    CubeInstance{{0.35F, -0.05F, -3.2F},
                 {1.4F, 1.4F, 1.4F},
                 0.28F,
                 0.52F,
                 0.95F}, // farther, blue
...
int main(int argc, char **argv) {
  try {
    ...
    if (!graphics.supportsDepthAttachmentFormat(depth_format)) {
      throw std::runtime_error(
          "Selected Rendering depth format failed support query.");
    }
    ...
    const auto applyReadyFrame = [&](const FrameBeginResult &frame) {
      ...
      const bool replace_graphics_state =
          !graphics_state.has_value() ||
          !graphics_state_color_format.has_value() ||
          *graphics_state_color_format != frame.color_format;
      const bool replace_depth = !depth_image.has_value() ||
                                 !depth_target.has_value() ||
                                 !depth_extent.has_value() ||
                                 depth_extent->width != frame.extent.width ||
                                 depth_extent->height != frame.extent.height;

      const bool existing_graphics_replaced =
          replace_graphics_state && graphics_state.has_value();
      const bool existing_depth_replaced =
          replace_depth &&
          (depth_target.has_value() || depth_image.has_value());
      ...
      if (replace_graphics_state) {
        ...
        graphics_state =
            makeGraphicsState(graphics, vertex_spirv, fragment_spirv,
    ...
    std::cout << "cubes=" << kCubes.size() << " depth_format=" << depth_format
              << '\n';
    std::cout
        << "hidden_surface_probe=draw_order_near_to_far_adversarial "
           "(without depth farther overwrites; with depth nearer still wins)\n";

    const Mat4 view =
        lookAt({0.0F, 0.35F, 1.8F}, {0.0F, 0.0F, -2.2F}, {0.0F, 1.0F, 0.0F});
    ...
    while (!window.shouldClose()) {
      ...
      if (!graphics_state.has_value() || !depth_target.has_value()) {
        throw std::logic_error(
            "Ready frame has no compatible depth-capable frame state.");
      }
    ...
    std::cout << "game_3d=passed frames=" << rendered_frames
              << " cubes=" << kCubes.size()
